...
// espaço reservado para a versão comprimida (deflate) da variavel a
char b[BUF_SIZE];

// espaço reservado para a versão descomprimida (inflate) da variavel b
char c[BUF_SIZE];
...
int  deflate_proc (char *buf) {
    ...
    a = buf;
//    seta "a" como a entrada e "b" como a saída comprimida
...
void inflate_proc (char* buf) {
    ...
    // Descompressão do arquivo de fato
...
//          fprintf(stderr, "read from shell %s %d\n", buf, (int)strlen(buf));
 ...
 //  printf ("p2childP[1]  %d\n", p2childP[1]);
   ...
   if(cprflag) {
       ...
       fprintf(stderr, "A mensagem recebida e descompactada possui %d bytes: %s\n", bytes, buf);
   }
    int i;
   for ( i=0; i<bytes; i++) {
          if (buf[i] == 0x04){
